...
const bb = {
  ...
  sequence(...specPairs) {
    return {type: 'sequence', specPairs};
  }
}
...
function decodeSequence(spec, encodedData, index, bit) {
  const data = {};
  for (let entry of spec.specPairs) {
...
function decodeArray(spec, encodedData, index, bit) {
...
function decodeBit(spec, encodedData, index, bit) {
  const startBit = bit;
  const { length } = spec;
  ...
  const data = parseInt(binary.substr(startBit, length), 2);
  return {data, index, bit};
}

function decodeUint8(spec, encodedData, index, bit) {
  if (bit != 0) {
    throw "Can't parse uint8 across byte boundary.";
  }
  const data = encodedData[index++];
  return {data, index, bit};
}
...
function _decode(spec, encodedData, index, bit) {
  return decoders[spec.type](spec, encodedData, index, bit);
}
