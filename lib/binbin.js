...
function encodeUint(spec, data, encodedData, bit, _currentByte) {
  if (bit != 0) {
...
function _encode(spec, data, encodedData, bit, currentByte) {
  return encoders[spec.type](spec, data, encodedData, bit, currentByte);
}

function encode(spec, data) {
  const encodedData = [];
  _encode(spec, data, encodedData, 0, 0);
  return encodedData;
}
