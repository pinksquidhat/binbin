...
function decodeSequence({subspecs}, encodedData, index, bit, _siblings) {
  ...
  for (let [name, subspec] of subspecs) {
    let result;
    try {
      result = _decode(subspec, encodedData, index, bit, data);
    } catch (e) {
      throw `Error occurred while decoding '${name}': ${e}`
    }
    index = result.index;
    bit = result.bit;
...
function decodeArray({length, subspec}, encodedData, index, bit, siblings) {
  ...
  for (let i = 0; i < length; i++) {
    let result;
    try {
      result = _decode(subspec, encodedData, index, bit);          
    } catch (e) {
      throw `Error occurred while decoding array index ${i}: ${e}`
    }
    index = result.index;
    bit = result.bit;
